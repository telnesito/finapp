...
const Navbar = ({ page }) => {
  ...
  return (
    <div className='flex gap-[10px] fixed w-full h-[72px] items-center justify-evenly bottom-0 bg-white'>
      ...
      {NAVBAR.map(({ page: pageIcon, icon: Icon, activeIcon: ActiveIcon }, index) =>

        <button onClick={() => router.push(pageIcon.toLocaleLowerCase())} key={index} type='button' className='active:bg-[#00000010] transition-all w-[60px] p-2 rounded flex flex-col items-center '>
          {page === pageIcon ? <>
